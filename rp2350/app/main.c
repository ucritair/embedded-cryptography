...
{
    static msg_payload_sensor_data_response_t response;

    // Validate payload size first
    if (msg->len != sizeof(msg_payload_sensor_data_t)) {
        printf("ERROR: Invalid sensor data payload size: %u (expected %zu)\n",
               msg->len, sizeof(msg_payload_sensor_data_t));

        response.status = SENSOR_DATA_STATUS_INVALID_PAYLOAD;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);
    ...
    if (!g_auth_token_valid) {
        printf("ERROR: No valid auth token. Run ZKP authentication first.\n");

        response.status = SENSOR_DATA_STATUS_NO_AUTH_TOKEN;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);

        printf("Sent response: status=%u (NO_AUTH_TOKEN)\n", response.status);
        return TF_STAY;
    }
    ...
    if (!config || !config->valid) {
        ...
        if (!config || !config->valid) {
            ...
            response.status = SENSOR_DATA_STATUS_NO_TFHE_CONFIG;
            msg->data = (const uint8_t *)&response;
            msg->len = sizeof(response);
            TF_Respond(tf, msg);

            printf("Sent response: status=%u (NO_TFHE_CONFIG)\n", response.status);
            return TF_STAY;
        }
    }
    ...
    // Double-check auth token (redundant but safe)
    ...
    if (crypto_shared == NULL) {
        ...
        response.status = SENSOR_DATA_STATUS_SHARED_MEM_ERROR;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);

        printf("Sent response: status=%u (SHARED_MEM_ERROR)\n", response.status);
        return TF_STAY;
    }
    ...
    while (!crypto_shared->compute_done) {
        ...
        if (timeout_count > 120) { // 60 second timeout
            ...
            response.status = SENSOR_DATA_STATUS_ENCRYPTION_TIMEOUT;
            msg->data = (const uint8_t *)&response;
            msg->len = sizeof(response);
            TF_Respond(tf, msg);

            printf("Sent response: status=%u (ENCRYPTION_TIMEOUT)\n", response.status);
            return TF_STAY;
        }
    }
    ...
    if (crypto_shared->error_code != 0) {
        ...
        response.status = SENSOR_DATA_STATUS_ENCRYPTION_FAILED;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);

        printf("Sent response: status=%u (ENCRYPTION_FAILED)\n", response.status);
        return TF_STAY;
    }
    ...
    if (ingest_result == 0) {
        ...
        response.status = SENSOR_DATA_STATUS_SUCCESS;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);

        printf("Sent response: status=%u (SUCCESS)\n", response.status);
    } else {
        printf("✗ Failed to ingest sensor data (error %d)\n", ingest_result);

        response.status = SENSOR_DATA_STATUS_INGEST_FAILED;
        msg->data = (const uint8_t *)&response;
        msg->len = sizeof(response);
        TF_Respond(tf, msg);

        printf("Sent response: status=%u (INGEST_FAILED)\n", response.status);
    }
