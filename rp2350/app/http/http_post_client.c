...
static int https_request_2(const char* hostname, const char* path, const char* method,
    ...
    if (g_current_auth_token) {
        // Find the position to insert the Authorization header (after Content-Type)
        char *content_type_pos = strstr(state.request_headers, "Content-Type: application/json\r\n");
        if (content_type_pos) {
            char *insert_pos = content_type_pos + strlen("Content-Type: application/json\r\n");
            
            // Create new headers with Authorization inserted
            static char temp_headers[HTTP_REQUEST_BUFFER_SIZE];
            size_t prefix_len = insert_pos - state.request_headers;
            
            snprintf(temp_headers, sizeof(temp_headers),
                "%.*sAuthorization: Bearer %s\r\n%s",
                (int)prefix_len, state.request_headers,
                g_current_auth_token,
                insert_pos);
            
            strcpy(state.request_headers, temp_headers);
            state.headers_len = strlen(state.request_headers);
        }
    }
